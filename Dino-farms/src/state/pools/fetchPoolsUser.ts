...
export const fetchUserStakeBalances = async (account) => {
  ...
  // Oyt / Oyt pool
  const { amount: masterPoolAmount } = await masterChefContract.methods.userInfo('5', account).call()
...
export const fetchUserPendingRewards = async (account) => {
  ...
  // Oyt / Oyt pool
  const pendingReward = await masterChefContract.methods.pendingdyt('5', account).call()
