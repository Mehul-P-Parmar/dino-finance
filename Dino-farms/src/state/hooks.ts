...
import { Toast, toastTypes } from '@dinofinance/uikit'
...
export const usePriceHtHusd = (): BigNumber => {
  ...
  const farm = useFarmFromPid(pid)
  const price = farm.tokenPriceVsQuote ? new BigNumber(1).div(farm.tokenPriceVsQuote) : ZERO
  console.log("HTPrice in Hooks", price)
  console.log("Farm in hooks", farm)
  return price
}
