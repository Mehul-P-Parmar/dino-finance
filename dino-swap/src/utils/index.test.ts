...
import { TokenAmount, Token, ChainId, Percent, JSBI } from '@dinofinance/sdk'
...
describe('utils', () => {
  describe('#getEtherscanLink', () => {
    it('correct for tx', () => {
      expect(getHecoScanLink(1, 'abc', 'transaction')).toEqual('https://hecoinfo.com/tx/abc')
    })
    it('correct for token', () => {
      expect(getHecoScanLink(1, 'abc', 'token')).toEqual('https://hecoinfo.com/token/abc')
    })
    it('correct for address', () => {
      expect(getHecoScanLink(1, 'abc', 'address')).toEqual('https://hecoinfo.com/address/abc')
    })
    it('unrecognized chain id defaults to mainnet', () => {
      expect(getHecoScanLink(2, 'abc', 'address')).toEqual('https://hecoinfo.com/address/abc')
    })
    it('ropsten', () => {
      expect(getHecoScanLink(3, 'abc', 'address')).toEqual('https://hecoinfo.com/address/abc')
    })
    it('enum', () => {
      expect(getHecoScanLink(ChainId.RINKEBY, 'abc', 'address')).toEqual('https://hecoinfo.com/address/abc')
    })
  })
